...
namespace Maticsoft.BuilderDALELParam
{
	public class BuilderDAL : IBuilderDAL
	{
		...
		public Hashtable Languagelist
		{
			get
			{
				return Language.LoadFromCfg("BuilderDALELParam.lan");
			}
		}
		...
		public string CreatExists()
		{
			...
			if (this._keys.Count > 0)
			{
				...
				stringPlus.AppendSpace(3, "strSql.Append(\"select count(1) from " + this._tablename);
				stringPlus.AppendLine(" where " + this.GetWhereExpression(this.Keys) + "\");");
		...
		public string CreatAdd()
		{
			...
			foreach (ColumnInfo current in this.Fieldlist)
			{
				...
				bool arg_1CA_0 = current.IsIdentity;
				string arg_1D2_0 = current.Length;
				if (!current.IsIdentity)
				{
			...
			else
			{
				if (text == "bool")
				{
					...
					stringPlus.AppendSpaceLine(4, "return false;");
					stringPlus.AppendSpaceLine(3, "}");
					stringPlus.AppendSpaceLine(3, "return true;");
				}
				else
				{
					if ((this.dbobj.DbType == "SQL2000" || this.dbobj.DbType == "SQL2005" || this.dbobj.DbType == "SQL2008" || this.dbobj.DbType == "SQL2012") && this.IsHasIdentity)
		...
		public string CreatUpdate()
		{
			...
			foreach (ColumnInfo current in this.Fieldlist)
			{
				string columnName = current.ColumnName;
				string typeName = current.TypeName;
				string arg_E7_0 = current.Length;
				bool arg_EE_0 = current.IsIdentity;
				bool arg_F5_0 = current.IsPrimaryKey;
			...
			stringPlus.DelLastComma();
			stringPlus.AppendLine("\");");
			stringPlus.AppendSpaceLine(3, "strSql.Append(\" where " + this.GetWhereExpression(this.Keys) + "\");");
			stringPlus.AppendSpaceLine(3, "Database db = DatabaseFactory.CreateDatabase();");
			stringPlus.AppendSpaceLine(3, "DbCommand dbCommand = db.GetSqlStringCommand(strSql.ToString());");
		...
		public string CreatReaderBind()
		{
			...
			foreach (ColumnInfo current in this.Fieldlist)
			{
				...
				string arg_D6_0 = current.Length;
				string key;
				switch (key = CodeCommon.DbTypeToCS(typeName))
				{
				case "int":
